...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (!jogoAtivo) {
            ...
            Color corFinal = modoNoite ? (Color){200, 200, 255, 255} : (Color){255, 255, 150, 255};
            DrawText(textoFinal, (larguraTela - larguraTextoFinal) / 2, alturaTela / 2 + 0, 20, corFinal);

            int larguraTextoReinicio = MeasureText("Pressione ESPACO para reiniciar", 20);
            Color corReinicio = modoNoite ? (Color){180, 180, 220, 255} : (Color){255, 255, 180, 255};
            DrawText("Pressione ESPACO para reiniciar", (larguraTela - larguraTextoReinicio) / 2, alturaTela / 2 + 30, 20, corReinicio);
